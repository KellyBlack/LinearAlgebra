...
\title{Section 5.5: Complex Eigenvalues}
\subtitle{Spinning In Circles?}
...
\begin{frame}{Goals}
  ...
  \begin{itemize}
  \item Determine complex valued eigenvalues and eigenvectors of a
    given matrix.
  \end{itemize}
...
\begin{frame}{Eigenvalues}
  ...
  \only<1>{A polynomial of degree $n$ has $n$ roots.}

  \only<2>{\dotfield{60}{24}}
...
\begin{frame}{Example}
  ...
  \only<1>{\dotfield{60}{24}}
  \only<2>{\dotfield{60}{24}}
  \only<3>{\dotfield{60}{24}}
  \only<4>{\dotfield{60}{24}}
\end{frame}


\section{Special Case: Rotation Matrices}

\begin{frame}{Rotation Matrices}

  \begin{columns}
    \column{0.7\textwidth}
    \includegraphics[width=5cm]{rotation}
    \column{0.3\textwidth}
    \begin{eqnarray*}
...
\begin{frame}{Complex Conjugates}
  ...
  If a real valued  matrix has complex eigenvalues then
  the complex conjugates of the eigenvalues are also
  eigenvalues. (Same goes for the eigenvectors.)
...
\begin{frame}{Complex Conjugates}
  ...
  If a matrix has an eigenvalue $9-\sqrt{7}i$ associated with the
  eigenvector $\columnVector{2-i \\ 5 \\ 7+2i \\ 6+10i}$, then find
  one other eigenvalue and one other eigenvector.
